...
4. In Capexity, connect Google Sheets storage if it is not already connected. Click **Voice**, enter the **separate voice access token**, then speak a command. The token stays in this browser tab's session storage; it is not published with the app.
